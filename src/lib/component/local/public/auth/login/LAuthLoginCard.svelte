...
<DaisyUiCard className="auth-card d-card-compact shadow-lg">
	<DaisyUiCardBody>
		...
		<form
			...
			action="?/signInEmail"
			use:enhance
			class="auth-form mt-4 space-y-4"
		>
			<input
				type="hidden"
				name="callbackURL"
				value={WebRoutesEnum.LANDING_PAGE}
			/>
			...
			<DaisyUiButton
				type="submit"
				className="d-btn-primary d-btn-block"
			>
				{m.log_in_submit()}
			</DaisyUiButton>
		</form>
		...
		<div class="auth-divider my-6 flex items-center gap-3">
			<span class="d-divider d-divider-vertical flex-1"
			></span>
		...
		<form method="post" action="?/signInSocial" use:enhance>
			...
			<DaisyUiButton
				type="submit"
				className="d-btn d-btn-outline d-btn-block gap-2"
			>
				<svg
					class="size-5"
					role="img"
		...
		<p
			class="mt-6 text-center text-sm text-base-content/80"
		>
			{m.dont_have_account()}
			<DaisyUiLink
				href={WebRoutesEnum.SIGNUP}
				className="d-link d-link-primary d-link-hover ml-1 font-medium"
			>
				{m.sign_up()}
			</DaisyUiLink>
		</p>
